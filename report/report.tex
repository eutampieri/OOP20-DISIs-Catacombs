...
\begin{document}
    ...
    \par
        \begin{tikzpicture}
            \tikzstyle{every node}=[font=\scriptsize]
            \begin{class}[text width=3.5cm]{MenuState}{-10, 0}

                \operation{+ update(delta : long)}
                \operation{+ render(g2 : Graphics2D)}
            \end{class}

            \begin{interface}[text width=3.5cm]{LogicMenu}{-10, -5}
            ...
            \begin{class}[text width=3.5cm]{LogicMenuImpl}{-10, -10}
                \implement{LogicMenu}
            ...
            \begin{class}[text width=3.5cm]{EndGameState}{10, 0}

                \operation{+ update(delta : long)}
                \operation{+ render(g2 : Graphics2D)}
            \end{class}

            \begin{interface}[text width=3.5cm]{EndGameLogic}{10, -5}

                \operation{continueGame(delta : long)}
                \operation{getFont( )}
                \operation{isBlink( )}
            \end{interface}

            \aggregation{EndGameState}{uses}{1}{EndGameLogic}

            \begin{class}[text width=3.5cm]{EndGameLogicImpl}{10, -10}
                \implement{EndGameLogic}
                \operation{+ continueGame(delta : long)}
                \operation{+ getFont( )}
                \operation{+ isBlink( )}
