...
\begin{document}
    ...
    \par
        \begin{tikzpicture}
            \tikzstyle{every node}=[font=\scriptsize]
            \begin{class}[text width=3.5cm]{MenuState}{-5, -0.2}
    ...
    \par
            \begin{tikzpicture}
                ...
                \begin{class}[text width=5cm]{AssetManager}{-2.5, 0}
                    \attribute{SINGLETON_MANAGER : AssetManager}

                    \operation{- AssetManager( )}
                    \operation{+ getAssetManager( )}
                \end{class}

                \begin{class}[text width=5cm]{KeyManager}{-2.5, 0}
                    \attribute{SINGLETON_KEYMANAGER : KeyManager}
    ...
    \par
            \begin{tikzpicture}
                ...
                \begin{abstractclass}[text width=3.5cm]{State}{-5, 0}

                    \operation{+ update(delta : long)}
                    \operation{+ render(g2 : Graphics2D)}
                \end{abstractclass}

                \begin{interface}[text width=3.5cm]{TransitionFactory}{5, 0}

                    \operation{transState(message : String, game : DungeonGame, state : State)}
                \end{interface}


                \begin{class}[text width=3.5cm]{TransitionFactoryImpl}{5, -5}
