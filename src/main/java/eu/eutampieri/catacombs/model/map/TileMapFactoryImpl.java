...
public class TileMapFactoryImpl implements TileMapFactory {
     ...
     * @param maxRoomDist maximum distance with the closest room's center for each room center
     * @return A Tilemap with nRooms square rooms connected by corridors in a tree, plus some random corridors
     * minRoomDist > maxRoomSide is recommended
     */
    private TileMap normal(int nRooms, int minRoomSide, int maxRoomSide, int minRoomDist, int maxRoomDist) {
        if(nRooms<=0) throw new IllegalArgumentException();
        if(minRoomSide<=0) throw new IllegalArgumentException();
        if(maxRoomSide<minRoomSide) throw new IllegalArgumentException();
        if(minRoomDist<0) throw new IllegalArgumentException();
        if(maxRoomDist<minRoomDist) throw new IllegalArgumentException();
        var pool = new ArrayList<Point>(); // pool of all points at acceptable distances frome the already selected rooms
            ...
            for(int dy=-maxRoomDist; dy<=maxRoomDist; dy++) { // recalculate distances of all points in range
                for(int dx=Math.abs(dy)-maxRoomDist; dx<=maxRoomDist-Math.abs(dy); dx++) {
                    ...
                    if(currentDist<formerDist) {
                        ...
                        if(formerDist>maxRoomDist) pool.add(cp); // add points that were too far before
                    }
                }
            }
